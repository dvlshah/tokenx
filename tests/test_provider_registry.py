...
class TestProviderRegistry:
    ...
    def test_concurrent_provider_registration(self):
        ...
        def register_provider_thread(provider_id: int):
            ...
            try:
                ...
                class ConcurrentTestAdapter(ProviderAdapter):
                    ...
                    def usage_from_response(self, response: Any) -> Usage:
                        return Usage(input_tokens=self.provider_id, output_tokens=self.provider_id, cached_tokens=0)
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [executor.submit(register_provider_thread, i) for i in range(num_threads)]
    ...
    def teardown_method(self):
        ...
        test_providers = [name for name in _PROVIDER_REGISTRY.keys() if name.startswith("test") or "test" in name]
